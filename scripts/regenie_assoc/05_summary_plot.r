...
if (filter_to_sv) {
  plot_output_path <- file.path(output_dir, "sv_snv.rsq0.8_maf0.005.regenie_results/summary_plots/strict_sv_only_plot.pdf")
  rdata_output_path <- file.path(output_dir, "sv_snv.rsq0.8_maf0.005.regenie_results/summary_plots/strict_sv_only_plot.RData")
} else {
  plot_output_path <- file.path(output_dir, "sv_snv.rsq0.8_maf0.005.regenie_results/summary_plots/strict_all_sig_plot.pdf")
}
...
pheno_group_df <- read_csv(pheno_meta_file)

# Assign numeric position to each phenotype (for y-axis control)
pheno_group_df <- pheno_group_df %>%
...
for (pheno in pheno_group_df$PHENOTYPE) {
  ...
  if (file.exists(file_name)) {
    ...
    warning(paste("File not found for phenotype:", pheno, "at", file_name))
  }
}

# Combine all significant results into a single data frame
sig_df <- bind_rows(all_sig_dfs)


df <- left_join(sig_df, pheno_group_df, by = "PHENOTYPE")
...
if (filter_to_sv) {
  ...
  save(plt, df, group_bands, file = rdata_output_path)
  # saveRDS(plt, file = rdata_output_path)
  # write.csv(df, file = "/staging/biology/u4432941/sv/imputation/summary_plot/sv_only_plot.csv", row.names = FALSE)
  # write.csv(group_bands, file = "/staging/biology/u4432941/sv/imputation/summary_plot/sv_group_bands.csv", row.names = FALSE)
  cat(paste0("R plot object saved to: ", rdata_output_path, "\n"))
} else {
  cat("No R plot object saved as filter_to_sv is FALSE.\n")
}

pdf(plot_output_path, width = 15, height = 8)
print(plt)
dev.off()

cat(paste0("Plot saved to: ", plot_output_path, "\n"))
