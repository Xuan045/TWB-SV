"""
Combine and aggregate REGENIE GWAS summary statistics.

Usage:
    python 04_1_combine_results.py \
        --phenotype <phenotype_name> \
        --panel <panel_name>

Logic:
    - Iterate through all chromosomes (1-22).
    - Read REGENIE output files matching the specified phenotype and panel.
    - Use the external `sv_type_count` module to classify variant types (e.g., DEL, DUP, SNV).
    - Concatenate per-chromosome results into a single pandas DataFrame.
    - Write the combined DataFrame to a single aggregated file.

Outputs:
    <output_dir>/<phenotype_name>.regenie - Aggregated summary statistics for all chromosomes
"""
...
out_dir     = os.environ.get("OUT_DIR", os.path.join(project_dir, "outputs"))
base_work   = os.environ.get("WORK_DIR", "/work/u4432941/sv/regenie")

# ── Load helper function to classify variant types ────────────────────────────
spec = spec_from_file_location("sv_type_count", os.path.join(project_dir, "scripts/analysis/02_sv_type_count.py"))
...
staging_dir   = out_dir
work_dir      = os.path.join(base_work, target_panel)
regenie_dir   = os.path.join(staging_dir, f"{target_panel}_regenie")
output_dir    = os.path.join(staging_dir, f"{target_panel}.{prefix}.regenie_results")
