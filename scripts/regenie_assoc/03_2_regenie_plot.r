...
# ==============================================================================
# Generate Manhattan and QQ plots for REGENIE association results.
#
# Usage:
#     Rscript 03_2_regenie_plot.r <phenotype> <panel> <sv_only>
#
# Logic:
#     - Read the combined .regenie association results.
#     - Filter to SV variants if sv_only is TRUE.
#     - Generate a Manhattan plot coloring variants by type.
#     - Generate a QQ plot to assess genomic inflation (lambda_gc).
#     - Save variants with P < 1e-5 to a text file.
#
# Outputs:
#     <OUT_DIR>/<prefix>.manhattan_plot.sv_only/<phenotype>_manhattan_plt.pdf
#     <OUT_DIR>/<prefix>.manhattan_plot.sv_only/<phenotype>_qq_plt.pdf
# ==============================================================================
...
wkdir <- Sys.getenv("OUT_DIR", "/staging/biology/u4432941/sv/prs/outputs")
file <- file.path(wkdir, paste0(prefix, ".regenie_results"), paste0(pheno, ".regenie"))
# /staging/biology/u4432941/sv/prs/outputs/sv_snv.rsq0.8_maf0.005.regenie_results/[PHENO].regenie
...
if (sv_only) {
  ...
  outdir <- file.path(wkdir, paste0(prefix, ".manhattan_plot.sv_only"))
  ...
  # Keep all variants
  outdir <- file.path(wkdir, paste0(prefix, ".manhattan_plot.all_variants"))
  if (!dir.exists(outdir)) {
    dir.create(outdir, recursive = TRUE)
  }
  prefix <- paste0(outdir, "/", pheno, "_")
