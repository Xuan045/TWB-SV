...
PHENO="$PHENO_FILE"
JDIR=${WORK_DIR:-/work/u4432941/sv}
...
# ---- Log ----
COMBINED_LOG="${LOG_DIR}/REGENIE_S2.${TARGET}.${TYPE}.chr${CHR}.log"
exec > "$COMBINED_LOG" 2>&1

echo "Starting Step 2 | ${TYPE} | chr${CHR} at $(date)"

# ---- pred list from Step 1 output ----
PRED_LIST="${OUT}/regenie_step1_${TYPE}_pred.list"
...
regenie \
    ...
    $EXTRA_PARAMS \
    --pred $PRED_LIST \
    --out ${OUT}/rsq${step2_r2_cutoff}_maf${step2_maf_cutoff}.regenie_step2_${TYPE}_chr${CHR} \
